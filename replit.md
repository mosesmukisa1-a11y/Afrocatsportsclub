...
- **Player Interviews** (`/interviews`): Magazine-style Q&A interview section with engaging card grid UI. `playerInterviews` table: questions/answers arrays, tags, featured flag, `likesCount`, `viewCount`. `interviewLikes` table tracks per-user likes (prevents double-liking). CRUD endpoints: `GET/POST /api/interviews`, `GET/PATCH/DELETE /api/interviews/:id`. New: `POST /api/interviews/:id/like` (toggle like), `POST /api/interviews/:id/view` (increment views), `GET /api/interviews-my-likes` (bulk like check). ADMIN/MANAGER/COACH can create/edit/delete/feature; all roles can like and view. UI: Player Spotlight hero (top-liked/featured), Trending bar (top 5 by likes), 3-column responsive card grid with avatar/jersey badge, Q1 preview, reading time, like/view counts, tag color chips. Full detail modal with video player (native mp4/webm or YouTube/Vimeo iframe), Q&A thread view, share-to-clipboard, view auto-increment. Form: tag multi-picker, camera recording (VideoRecorder) or URL input, Q&A pairs builder, featured toggle. Filters: search text, tag dropdown, video/text type, featured toggle.
...
- **Finance Module** (`/finance`): Full financial management with tabs — Summary, Payments, Expenses, Player Finance, Fee Config, General Ledger, Valuations. Player Finance now shows proper Transfer Value (not just club investment) using the full FIFA-style valuation formula: `Base × Age Mult × (1 + Perf/100) × (1 + Attend×0.4) + Club Investment`. Valuation breakdown section explains each factor with descriptions. Fee defaults: Non-Working N$400, Working N$800, Dev N$2500, Resource N$1500, League N$0. Approval roles: ADMIN/FINANCE. Management roles: ADMIN/FINANCE/MANAGER.
- **Finance PDF Reports** (`server/financePdf.ts`): Three PDF report types generated with pdf-lib:
  - **Individual Player** (`GET /api/finance/player/:id/valuation-pdf`): Player info, fee breakdown, valuation breakdown with 5-factor explanation (base/age/performance/attendance/club investment), payment history, stats summary.
  - **Team** (`GET /api/finance/team/:teamId/valuation-pdf`): Team roster with all player values, summary totals, valuation methodology section.
  - **Club** (`GET /api/finance/club/valuation-pdf`): Full club overview, team-by-team breakdown, complete player register, grand totals. Dark-themed with teal/gold accents.
  - PDF buttons: Player Finance tab has "Download Player Finance PDF"; Valuations tab has per-row PDF icon, Team PDF selector dropdown, and "Club PDF" gold button.
- **Transfer Value Fix**: `/api/finance/player/:id` now calls `buildPlayerValuation()` from `financePdf.ts` returning `transferValue`, `valuationBreakdown`, `stats`, `perfScore`, `attendRate`, `ageMult`. Previously returned `clubValue: 0` (was using `playerValue` not computed).
