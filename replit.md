...
- **Self-Registration with Role Selection**: POST /api/auth/register accepts `role` field (PLAYER, COACH, STATISTICIAN, MEDICAL, FINANCE). Registration page shows role selector grid. Player role shows team/position/jersey preferences. Non-player roles skip player record creation. All roles require admin approval (accountStatus: PENDING_APPROVAL). Admin can override role during approval.
- **Role-Based Login Redirects**: After login, users are redirected based on role: PLAYER → /player-dashboard, STATISTICIAN → /stats, FINANCE → /finance, MEDICAL → /injuries, ADMIN/MANAGER/COACH → /dashboard.
- **Coach/Admin Timely Reports**: Dashboard shows "Timely Reports" section for ADMIN/MANAGER/COACH roles with recent matches, result badges, and "New" indicators for matches within last 7 days. Links to full Reports page.
...
- **Global Dark Theme**: Entire app uses Afrocat dark palette. Tailwind v4 `@theme inline` block defines `afrocat-*` color tokens (`bg-afrocat-bg`, `text-afrocat-text`, `bg-afrocat-teal`, `text-afrocat-gold`, `bg-afrocat-glow`, `bg-afrocat-card` etc.). `:root` HSL variables set to dark values for shadcn components. CSS utility: `.afrocat-card` (gradient card with border+shadow). Background glow via `bg-afrocat-glow`. Global html/body bg: `#0B0F14`. Gold selection highlight, gold scrollbar thumb. Layout sidebar uses `bg-afrocat-card`.
- **Note**: Some secondary pages (Finance, Injuries, Contracts, Coaches, Matches, Reports, Players, Documents, AdminUsers) still use generic Tailwind color classes (bg-green-100, etc.) — these should be migrated to `afrocat-*` tokens for full consistency.
