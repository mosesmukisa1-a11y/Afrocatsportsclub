...
- **Enter Stats (Touch Stats)**: The ONLY stats entry method (`/stats`). Event-based touch entry: Tap Player → Action (Serve/Receive/Set/Attack/Block/Dig/Free Ball) → Outcome detail. Real-time feed with Undo. Live scoreboard with Points + Sets. Sync to player stats. Old manual bulk stats entry removed — all stats flow through touch events → sync. Roles: ADMIN, MANAGER, COACH, STATISTICIAN
